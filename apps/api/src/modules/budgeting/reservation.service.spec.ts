...
describe('ReservationService lifecycle', () => {
  it('createDraft does not touch the release balance at all', async () => {
    ...
    const reservation = await service.createDraft(organizationId, {
  ...
  it('editDraft updates the amount while still draft, and rejects once no longer draft', async () => {
    ...
    const reservation = await service.createDraft(organizationId, {
      budgetReleaseId: release.id,
      reservationAmount: 10_000,
    });

    const edited = await service.editDraft(organizationId, reservation.id, reservation.version, {
      reservationAmount: 15_000,
    });
    ...
    await expect(
      service.editDraft(organizationId, submitted.id, submitted.version, {
  ...
  it('submit rejects when the request exceeds available budget', async () => {
    ...
    await expect(
      service.submit(organizationId, reservation.id, reservation.version),
  ...
  it('rejects creating a draft reservation against a release not yet in "released" status', async () => {
    ...
    await expect(
      service.createDraft(organizationId, {
        budgetReleaseId: draftRelease.id,
        reservationAmount: 10_000,
      }),
  ...
  it('rejects any action with a stale version (optimistic concurrency)', async () => {
    ...
    const reservation = await service.createDraft(organizationId, {
      budgetReleaseId: release.id,
      reservationAmount: 5_000,
    });

    await expect(
      service.submit(organizationId, reservation.id, reservation.version + 1),
    ).rejects.toThrow();
