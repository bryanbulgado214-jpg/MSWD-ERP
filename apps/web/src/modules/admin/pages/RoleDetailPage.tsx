...
export function RoleDetailPage() {
  ...
  function load() {
    ...
    Promise.all([getRole(id), listPermissions()])
      .then(([r, p]) => { setRole(r); setAllPerms(p); })
  ...
  if (loading) return <div className="admin-page"><div className="admin-loading">Loading role...</div></div>;
  if (!role) return <div className="admin-page"><div className="admin-error">Role not found.</div></div>;
  ...
  const filteredPerms = filterModule ? availablePerms.filter((p) => p.module === filterModule) : availablePerms;
  ...
  return (
    <div className="admin-page">
      <div className="admin-header">
        <div>
          <Link to="/admin/roles" className="admin-back">&larr; Roles</Link>
          <h1 className="admin-title">{role.name} <span className="admin-title__code">({role.code})</span></h1>
      ...
      <div className="admin-section">
        <div className="admin-section__header">
          ...
          <button type="button" className="admin-btn admin-btn--primary admin-btn--sm" onClick={() => setShowAddPerm(true)}>
            + Add Permission
          </button>
        </div>
          ...
          <div className="admin-modal-overlay" onClick={() => setShowAddPerm(false)}>
            <div className="admin-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="admin-field" style={{ marginBottom: 8 }}>
                ...
                <select className="admin-input" value={filterModule} onChange={(e) => { setFilterModule(e.target.value); setSelectedPermId(''); }}>
              ...
              <div className="admin-field">
                ...
                <select className="admin-input" value={selectedPermId} onChange={(e) => setSelectedPermId(e.target.value)}>
              ...
              <div className="admin-modal__actions">
                <button type="button" className="admin-btn" onClick={() => setShowAddPerm(false)}>Cancel</button>
                <button type="button" className="admin-btn admin-btn--primary" disabled={!selectedPermId} onClick={handleAddPerm}>Add</button>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="admin-perm-list">
              {permsByModule[mod].map((p) => (
                <div key={p.assignmentId} className="admin-perm-item">
                  ...
                  <button type="button" className="admin-btn admin-btn--sm admin-btn--danger" onClick={() => handleRemovePerm(p.assignmentId)}>Remove</button>
                </div>
              ))}
            </div>
          </div>
