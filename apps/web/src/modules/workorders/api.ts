...
export async function assignWorkOrder(id: string, data: { expectedVersion: number; assignedTo: string }): Promise<WorkOrder> {
  const res = await authFetchMutate(`/workorders/${id}/assign`, 'POST', data);
  return res.json();
}

export async function startWorkOrder(id: string, data: { expectedVersion: number }): Promise<WorkOrder> {
  const res = await authFetchMutate(`/workorders/${id}/start`, 'POST', data);
  return res.json();
}

export async function completeWorkOrder(id: string, data: {
  expectedVersion: number;
...
export async function verifyWorkOrder(id: string, data: { expectedVersion: number }): Promise<WorkOrder> {
  const res = await authFetchMutate(`/workorders/${id}/verify`, 'POST', data);
  return res.json();
}

export async function cancelWorkOrder(id: string, data: { expectedVersion: number; reason?: string }): Promise<WorkOrder> {
...
export async function addWorkOrderMaterial(id: string, data: {
