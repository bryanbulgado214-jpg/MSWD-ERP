...
type LoadState =
  { status: 'loading' } | { status: 'error'; message: string } | { status: 'loaded'; data: Meter };

const METER_SIZES: Record<string, string> = {
  half_inch: '1/2"',
  three_quarter_inch: '3/4"',
  one_inch: '1"',
  one_half_inch: '1-1/2"',
  two_inch: '2"',
  three_inch: '3"',
...
export default function MeterDetailPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setState({
        status: 'error',
        message: err instanceof BillingApiError ? err.message : 'Failed to load meter.',
      });
    }
  }
  ...
  return (
    <div className="bill-page">
      <BillingSubNav />
      <Link to="/billing/meters" className="bill-back-link">
        &larr; Back to Meters
      </Link>
      ...
      {!editing && (
        <>
          <dl className="bill-detail-grid">
            ...
            <div className="bill-detail-field">
              <dt>Size</dt>
              <dd>{m.size ? METER_SIZES[m.size] || m.size.replace(/_/g, ' ') : '--'}</dd>
            </div>
            <div className="bill-detail-field">
              <dt>Initial Reading</dt>
              <dd className="bill-text-mono">{m.initialReading}</dd>
            </div>
            <div className="bill-detail-field">
              <dt>Last Reading</dt>
              <dd className="bill-text-mono">{m.lastReading ?? '--'}</dd>
            </div>
            <div className="bill-detail-field">
              <dt>Notes</dt>
              <dd>{m.notes || '--'}</dd>
            ...
            <tbody>
              {m.consumerMeters.map((cm) => (
                <tr key={cm.id}>
                  ...
                  <td>
                    {cm.consumer ? `${cm.consumer.lastName}, ${cm.consumer.firstName}` : '--'}
                  </td>
                  <td>
                    {cm.consumer ? (
                      <span className={`bill-badge bill-badge--${cm.consumer.status}`}>
                        {cm.consumer.status}
                      </span>
                    ) : (
                      '--'
