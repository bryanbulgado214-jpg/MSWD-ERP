...
import { getBillingSummary, getAgingReport, getCollectionSummary, getConsumers, getDisconnectionOrders } from '../api';
...
export default function BillingDashboardPage() {
    ...
    async function load() {
      try {
        const results: DashboardData = {
          consumerCount: 0, activeConsumers: 0, billingSummary: null,
          agingSummary: [], totalOutstanding: 0, recentCollections: null, pendingDisconnections: 0,
        };
        ...
        const [consumers, summary, aging, disconnections] = await Promise.allSettled([
          ...
          hasPermission('billing.disconnect.manage') ? getDisconnectionOrders('status=notice_issued') : Promise.resolve([]),
        ]);
        ...
        if (aging.status === 'fulfilled' && aging.value) {
          const a = aging.value as { summary: DashboardData['agingSummary']; totalOutstanding: number };
        ...
        if (hasPermission('billing.reports')) {
          try {
            ...
            const col = await getCollectionSummary(startOfMonth, today) as { totalCollected: number; paymentCount: number };
  ...
  if (loading) return <div className="bill-page"><BillingSubNav /><div className="bill-loading">Loading dashboard...</div></div>;
  if (!data) return <div className="bill-page"><BillingSubNav /><div className="bill-error">Failed to load dashboard data.</div></div>;
    ...
    <div className="bill-page">
      ...
      <div className="bill-dash-grid">
        <div className="bill-dash-card bill-dash-card--blue">
          ...
          <Link to="/billing/consumers" className="bill-dash-card__link">View all</Link>
        </div>

        <div className="bill-dash-card bill-dash-card--green">
          <div className="bill-dash-card__label">Total Collected</div>
          <div className="bill-dash-card__value">{formatPeso(data.billingSummary?.totalCollected ?? 0)}</div>
          <div className="bill-dash-card__sub">{collRate.toFixed(1)}% collection rate</div>
          <Link to="/billing/payments" className="bill-dash-card__link">Payments</Link>
        </div>
        ...
        <div className="bill-dash-card bill-dash-card--red">
          ...
          <div className="bill-dash-card__sub">{data.billingSummary?.totalBills ?? 0} total bills</div>
          <Link to="/billing/bills" className="bill-dash-card__link">View bills</Link>
        </div>

        <div className="bill-dash-card bill-dash-card--amber">
          <div className="bill-dash-card__label">This Month Collections</div>
          <div className="bill-dash-card__value">{formatPeso(data.recentCollections?.totalCollected ?? 0)}</div>
          <div className="bill-dash-card__sub">{data.recentCollections?.paymentCount ?? 0} payments</div>
          <Link to="/billing/reports" className="bill-dash-card__link">Reports</Link>
        </div>
      </div>
        ...
        <div className="bill-dash-panel">
          ...
          {data.agingSummary.length > 0 ? (
            <table className="bill-table" style={{ fontSize: '13px' }}>
              <thead>
                <tr><th>Bracket</th><th style={{ textAlign: 'right' }}>Bills</th><th style={{ textAlign: 'right' }}>Amount</th></tr>
              </thead>
              <tbody>
                {data.agingSummary.map((s) => (
                  <tr key={s.bracket}>
                    <td>{s.bracket === 'current' ? 'Current' : `${s.bracket} days`}</td>
                    <td className="bill-text-mono" style={{ textAlign: 'right' }}>{s.count}</td>
                    <td className="bill-text-mono" style={{ textAlign: 'right' }}>{formatPeso(s.total)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr style={{ fontWeight: 700 }}>
                  <td>Total</td>
                  <td className="bill-text-mono" style={{ textAlign: 'right' }}>{data.agingSummary.reduce((s, a) => s + a.count, 0)}</td>
                  <td className="bill-text-mono" style={{ textAlign: 'right' }}>{formatPeso(data.totalOutstanding)}</td>
                </tr>
              </tfoot>
            </table>
          ) : (
        ...
        <div className="bill-dash-panel">
          ...
          <div className="bill-dash-actions">
            {hasPermission('billing.payment.collect') && (
              <Link to="/billing/payments" className="bill-dash-action-btn">Collect Payment</Link>
            )}
            {hasPermission('billing.read') && (
              <Link to="/billing/readings" className="bill-dash-action-btn">Record Reading</Link>
            )}
            {hasPermission('billing.manage') && (
              <Link to="/billing/consumers" className="bill-dash-action-btn">Add Consumer</Link>
            )}
            {hasPermission('billing.reports') && (
              <Link to="/billing/reports" className="bill-dash-action-btn">Generate Report</Link>
            )}
            {hasPermission('billing.disconnect.manage') && (
              <Link to="/billing/disconnections" className="bill-dash-action-btn">
                Disconnections {data.pendingDisconnections > 0 && <span className="bill-badge bill-badge--notice_issued">{data.pendingDisconnections}</span>}
              </Link>
            )}
          </div>
        </div>
        ...
        <div className="bill-dash-panel" style={{ marginTop: 20 }}>
          ...
          <div className="bill-dash-status-bar">
            {data.billingSummary.byStatus.map((s) => {
              const pct = data.billingSummary!.totalBills > 0
                ? (s.count / data.billingSummary!.totalBills) * 100 : 0;
              return (
                <div key={s.status} className={`bill-dash-status-seg bill-dash-status-seg--${s.status}`}
        ...
        <div className="bill-dash-panel" style={{ marginTop: 20 }}>
          ...
          <div className="bill-dash-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
            <div style={{ textAlign: 'center', padding: '12px' }}>
              <div style={{ fontSize: '11px', color: '#667085', fontWeight: 600, textTransform: 'uppercase' }}>Total Consumption</div>
              <div style={{ fontSize: '22px', fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>{data.billingSummary.totalConsumption.toLocaleString()} cu.m.</div>
            </div>
            <div style={{ textAlign: 'center', padding: '12px' }}>
              <div style={{ fontSize: '11px', color: '#667085', fontWeight: 600, textTransform: 'uppercase' }}>Avg per Bill</div>
              <div style={{ fontSize: '22px', fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>
                {data.billingSummary.totalBills > 0
                  ? (data.billingSummary.totalConsumption / data.billingSummary.totalBills).toFixed(1)
                  : '0'} cu.m.
              </div>
            </div>
            <div style={{ textAlign: 'center', padding: '12px' }}>
              <div style={{ fontSize: '11px', color: '#667085', fontWeight: 600, textTransform: 'uppercase' }}>Avg Bill Amount</div>
              <div style={{ fontSize: '22px', fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>
                {formatPeso(data.billingSummary.totalBills > 0
                  ? data.billingSummary.totalBilled / data.billingSummary.totalBills : 0)}
              </div>
            </div>
          </div>
        </div>
