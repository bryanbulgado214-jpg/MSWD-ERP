...
import { getDashboardCounts, type CashierDashboardCounts } from '../cashierCollectionApi';
...
export default function BillingDashboardPage() {
  ...
  const [counts, setCounts] = useState<CashierDashboardCounts | null>(null);
  ...
  return (
    <div className="bill-page">
      ...
      {/* Cashier's worklist counts — each tile links to its list. */}
      {cashierHome &&
        (() => {
          const tiles = [
            {
              label: 'Checks Due for Printing',
              value: counts?.checksDueForPrinting,
              to: '/accounting/checks',
              color: 'amber',
              sub: 'Assign a number & print',
            },
            {
              label: 'Uncleared Checks',
              value: counts?.unclearedChecks,
              to: '/accounting/checks',
              color: 'blue',
              sub: 'Issued, not yet cleared',
            },
            {
              label: 'Undeposited Collections',
              value: counts?.undepositedCollections,
              to: '/billing/cashier-report',
              color: 'green',
              sub: 'Record the bank deposit',
            },
            {
              label: 'Collections Awaiting Posting',
              value: counts?.collectionsNotPosted,
              to: '/billing/cashier-report',
              color: 'red',
              sub: "For accountant's review",
            },
            {
              label: 'Deposits Awaiting Posting',
              value: counts?.depositsNotPosted,
              to: '/billing/cashier-report',
              color: 'red',
              sub: "For accountant's review",
            },
            ...
            <div className="bill-dash-grid">
              {tiles.map((t) => (
                <Link
                  key={t.label}
                  to={t.to}
                  className={`bill-dash-card bill-dash-card--${t.color}`}
                  style={{ textDecoration: 'none', color: 'inherit' }}
                >
                  <div className="bill-dash-card__label">{t.label}</div>
                  <div className="bill-dash-card__value">{t.value ?? '—'}</div>
                  <div className="bill-dash-card__sub">{t.sub}</div>
                </Link>
              ))}
            </div>
          );
              ...
              <div className="bill-dash-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
                ...
                <div style={{ textAlign: 'center', padding: '12px' }}>
                  ...
                  >
                    {data.billingSummary.totalBills > 0
                      ? (
                          data.billingSummary.totalConsumption / data.billingSummary.totalBills
                        ).toFixed(1)
                      : '0'}{' '}
                    cu.m.
                  </div>
