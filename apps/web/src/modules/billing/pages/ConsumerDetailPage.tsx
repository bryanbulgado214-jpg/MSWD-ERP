...
export default function ConsumerDetailPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setState({
        status: 'error',
        message: err instanceof BillingApiError ? err.message : 'Failed to load consumer.',
      });
    }
  }
  ...
  return (
    <div className="bill-page">
      ...
      >
        <Link to="/billing/consumers" className="bill-back-link">
      ...
      {!editing && (
        <>
          <dl className="bill-detail-grid">
            <div className="bill-detail-field">
              <dt>Account Number</dt>
              <dd>{c.accountNumber}</dd>
            </div>
            <div className="bill-detail-field">
              <dt>Type</dt>
              <dd>
                <span className={`bill-badge bill-badge--${c.consumerType}`}>{c.consumerType}</span>
              </dd>
            </div>
            <div className="bill-detail-field">
              <dt>Address</dt>
            ...
            <div className="bill-detail-field">
              ...
              <dd>{c.isSeniorCitizen ? 'Yes' : 'No'}</dd>
            </div>
            <div className="bill-detail-field">
              <dt>PWD</dt>
              <dd>{c.isPwd ? 'Yes' : 'No'}</dd>
            </div>
            <div className="bill-detail-field">
              <dt>Connection Date</dt>
              <dd>{c.connectionDate ? new Date(c.connectionDate).toLocaleDateString() : '--'}</dd>
            </div>
            {c.businessName && (
              <div className="bill-detail-field">
                <dt>Business Name</dt>
                <dd>{c.businessName}</dd>
              </div>
            )}
            <div className="bill-detail-field">
              <dt>Notes</dt>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            {hasPermission('billing.meter.manage') && (
              <button
                type="button"
                className="bill-btn bill-btn--success"
                onClick={openAssignMeter}
              >
                Assign Meter
              </button>
            )}
      ...
      {editing && (
        <form className="bill-form" onSubmit={handleUpdate}>
          ...
          <div className="bill-form-actions">
            <button type="button" className="bill-btn" onClick={() => setEditing(false)}>
              Cancel
            </button>
      ...
      {showAssignMeter && (
        <form className="bill-form" onSubmit={handleAssignMeter} style={{ marginTop: '16px' }}>
          ...
          <div className="bill-form-actions">
            <button type="button" className="bill-btn" onClick={() => setShowAssignMeter(false)}>
              Cancel
            </button>
