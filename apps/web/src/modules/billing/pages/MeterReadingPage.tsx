  ...
  return (
    <div className="bill-page">
      ...
      <div className="bill-toolbar">
        ...
        {selectedPeriodId &&
          hasPermission('billing.reading.manage') &&
          selectedPeriod &&
          (selectedPeriod.status === 'open' || selectedPeriod.status === 'reading') && (
            <button className="bill-btn bill-btn--primary" type="button" onClick={openEntry}>
              Record Reading
            </button>
          )}
        {selectedPeriodId &&
          hasPermission('billing.bill.generate') &&
          selectedPeriod &&
          (selectedPeriod.status === 'reading' || selectedPeriod.status === 'billing') && (
            <button
      ...
      {genResult && (
        <div
          className={genResult.startsWith('Generated') ? 'bill-success' : 'bill-error'}
        ...
        <form className="bill-form" onSubmit={handleSubmit}>
          ...
          {currentMeter && (
            <div style={{ fontSize: '12px', color: '#667085', margin: '4px 0 8px' }}>
              Meter: {currentMeter.serialNumber} | Consumption:{' '}
              {currentReading ? (Number(currentReading) - prevReading).toFixed(2) : '0.00'} cu.m.
            </div>
          )}
          ...
          <div className="bill-form-actions">
            <button type="button" className="bill-btn" onClick={() => setShowEntry(false)}>
              Cancel
            </button>
      ...
      {readings.status === 'idle' && (
        <div className="bill-empty">Select a billing period to view readings.</div>
      )}
            ...
            <tbody>
              {readings.data.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>
                    {r.consumer.lastName}, {r.consumer.firstName}
                  </td>
