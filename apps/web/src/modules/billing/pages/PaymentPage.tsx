...
import {
  BillingApiError, createPayment, getConsumers, getNextOrNumber,
  getPayments, getUnpaidBills, voidPayment,
} from '../api';
...
export default function PaymentPage() {
  ...
  async function loadPayments() {
    ...
    try {
      ...
      setPayments({ status: 'error', message: err instanceof BillingApiError ? err.message : 'Failed to load.' });
    }
  }
  ...
  async function openCollectForm() {
    ...
    setCheckNumber(''); setCheckDate(''); setBankName(''); setReferenceNumber('');
    setRemarks('');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (allocations.size === 0) { setFormError('Select at least one bill to apply payment to.'); return; }
    setSaving(true); setFormError('');
    try {
      const allocs = Array.from(allocations.entries()).map(([billId, amountApplied]) => ({ billId, amountApplied }));
      await createPayment({
        orNumber,
        ...
        ...((paymentMethod === 'online' || paymentMethod === 'bank_deposit') && referenceNumber ? { referenceNumber } : {}),
        ...(remarks ? { remarks } : {}),
        allocations: allocs,
      });
  ...
  async function handleVoid(payment: PaymentListItem) {
    ...
    try {
      await voidPayment(payment.id, { expectedVersion: payment.version, voidReason: voidReason.trim() });
      setVoidingId('');
      setVoidReason('');
      loadPayments();
    } catch (err) {
  ...
  const filteredConsumers = consumerSearch.length >= 2
    ? consumers.filter((c) =>
  ...
  return (
    <div className="bill-page">
      ...
      {successMsg && <div className="bill-success" style={{ marginBottom: '12px' }}>{successMsg}</div>}
            ...
            <div>
              ...
              {filteredConsumers.length > 0 && (
                <table className="bill-table" style={{ maxWidth: '600px' }}>
                  <thead>
                    <tr><th>Account #</th><th>Name</th><th>Type</th><th></th></tr>
                  </thead>
                  <tbody>
                    {filteredConsumers.map((c) => (
                      <tr key={c.id}>
                        ...
                        <td>
                          <button type="button" className="bill-btn bill-btn--primary" style={{ padding: '4px 10px', fontSize: '12px' }}
              ...
              <div className="bill-form-actions">
                <button type="button" className="bill-btn" onClick={() => setShowCollect(false)}>Cancel</button>
              </div>
            </div>
          ) : (
            <div>
              <div style={{ background: '#eff8ff', padding: '10px 14px', borderRadius: '6px', marginBottom: '16px', fontSize: '13px' }}>
                <strong>{selectedConsumer.accountNumber}</strong> — {selectedConsumer.lastName}, {selectedConsumer.firstName}
                {' '}
                <button type="button" style={{ fontSize: '12px', color: '#175cd3', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}
              ...
              {unpaidBills.length === 0 ? (
                <div className="bill-empty" style={{ padding: '24px 0' }}>No unpaid bills for this consumer.</div>
              ) : (
                <div style={{ marginBottom: '16px' }}>
                  <label style={{ fontSize: '12px', fontWeight: 600, color: '#475467', textTransform: 'uppercase', letterSpacing: '0.03em' }}>
                    Bills to Pay
                  </label>
                  <table className="bill-table" style={{ marginTop: '6px' }}>
                        ...
                        <tr key={b.id}>
                          ...
                          <td style={{ width: '120px' }}>
                            <input
                              ...
                              onChange={(e) => setAllocationAmount(b.id, parseFloat(e.target.value) || 0)}
                              style={{ width: '100px', padding: '4px 8px', fontSize: '13px', fontVariantNumeric: 'tabular-nums' }}
                            />
                          </td>
                        </tr>
                      ))}
                    ...
                    <tfoot>
                      <tr style={{ fontWeight: 700 }}>
                        <td colSpan={6} style={{ textAlign: 'right' }}>Total Payment:</td>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                <div className="bill-field">
                  ...
                  <input required value={orNumber} onChange={(e) => setOrNumber(e.target.value)} className="bill-text-mono" />
                </div>
                <div className="bill-field">
              ...
              <div className="bill-form-actions">
                <button type="button" className="bill-btn" onClick={() => setShowCollect(false)}>Cancel</button>
                <button type="submit" className="bill-btn bill-btn--primary" disabled={saving || totalAllocation <= 0}>
          ...
          <tbody>
            {payments.data.map((p) => (
              <tr key={p.id} style={p.status === 'voided' ? { opacity: 0.5 } : undefined}>
                <td>
                  <Link to={`/billing/payments/${p.id}`} className="bill-table__link bill-text-mono">
                    {p.orNumber}
                  </Link>
                </td>
                ...
                <td className="bill-text-mono">{p.allocations.map((a) => a.bill.billNumber).join(', ')}</td>
                <td>
                  <span className={`bill-badge bill-badge--${p.status}`}>{p.status}</span>
                </td>
                <td>
                  {canVoid && p.status === 'valid' && voidingId !== p.id && (
                    <button type="button" className="bill-btn bill-btn--danger" style={{ padding: '3px 8px', fontSize: '11px' }}
                  ...
                  {voidingId === p.id && (
                    <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                      ...
                      />
                      <button type="button" className="bill-btn bill-btn--danger" style={{ padding: '3px 8px', fontSize: '11px' }}
                        onClick={() => handleVoid(p)} disabled={!voidReason.trim()}>
                        Confirm
                      </button>
                      <button type="button" className="bill-btn" style={{ padding: '3px 8px', fontSize: '11px' }}
