...
import {
  BillingApiError, applyPenalties, createDisconnectionOrder,
  getConsumersInArrears, getDisconnectionOrders, transitionDisconnection,
} from '../api';
...
export default function DisconnectionPage() {
  ...
  async function loadOrders() {
    ...
    try {
      const data = await getDisconnectionOrders(statusFilter ? `status=${statusFilter}` : undefined);
      setOrders({ status: 'loaded', data });
    } catch (err) {
      setOrders({ status: 'error', message: err instanceof BillingApiError ? err.message : 'Failed.' });
    }
  }
  ...
  async function loadArrears() {
    ...
    try {
      ...
      setArrears({ status: 'error', message: err instanceof BillingApiError ? err.message : 'Failed.' });
    }
  }
  ...
  function getActionButtons(order: DisconnectionOrderListItem) {
    ...
    if (order.status === 'notice_issued') actions.push({ action: 'serve', label: 'Mark Served', cls: '' });
    if (order.status === 'served') actions.push({ action: 'disconnect', label: 'Disconnect', cls: 'bill-btn--danger' });
    if (order.status === 'disconnected') actions.push({ action: 'reconnect', label: 'Reconnect', cls: 'bill-btn--success' });
  ...
  return (
    <div className="bill-page">
      ...
      <div className="bill-toolbar">
        ...
        {canManage && (
          <button type="button" className="bill-btn bill-btn--danger" onClick={handleApplyPenalties}
      ...
      {penaltyResult && (
        <div className={penaltyResult.startsWith('Penalties') ? 'bill-success' : 'bill-error'}
          style={{ marginBottom: '12px' }}>{penaltyResult}</div>
      )}

      {showCreate && (
        <form className="bill-form" onSubmit={handleCreate}>
          <h3 style={{ margin: '0 0 16px', color: '#0a2540', fontSize: '16px' }}>Create Disconnection Notice</h3>
          ...
          <div className="bill-field" style={{ maxWidth: '400px' }}>
            <label>Remarks</label>
            <input value={createRemarks} onChange={(e) => setCreateRemarks(e.target.value)} placeholder="Optional" />
          </div>
          <div className="bill-form-actions">
            <button type="button" className="bill-btn" onClick={() => setShowCreate(false)}>Cancel</button>
        ...
        <>
          <div style={{ marginBottom: '12px' }}>
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}
              style={{ padding: '6px 10px', borderRadius: '6px', border: '1.5px solid #d0d5dd', fontSize: '13px' }}>
            ...
            <table className="bill-table">
              ...
              <tbody>
                {orders.data.map((o) => (
                  <tr key={o.id}>
                    <td>
                      <Link to={`/billing/disconnections/${o.id}`} className="bill-table__link bill-text-mono">
                        {o.orderNumber}
                      </Link>
                    </td>
                    <td>
                      <Link to={`/billing/consumers/${o.consumerId}`} className="bill-table__link">
                    ...
                    <td className="bill-text-mono" style={{ fontWeight: 600 }}>{formatPeso(o.totalArrears)}</td>
                    <td>
                    ...
                    <td>
                      {actionId === o.id ? (
                        <div style={{ display: 'flex', gap: '4px', alignItems: 'center', flexWrap: 'wrap' }}>
                          {actionType === 'cancel' && (
                            <input placeholder="Reason..." value={actionReason}
                              onChange={(e) => setActionReason(e.target.value)}
                              style={{ width: '120px', padding: '3px 6px', fontSize: '12px' }} />
                          )}
                          {actionType === 'reconnect' && (
                            <input placeholder="Reconn. fee" type="number" value={reconnFee}
                          ...
                          <button type="button" className="bill-btn bill-btn--primary"
                            ...
                            onClick={() => handleAction(o)}>Confirm</button>
                          <button type="button" className="bill-btn"
                            style={{ padding: '3px 8px', fontSize: '11px' }}
                            onClick={() => setActionId('')}>Cancel</button>
                        </div>
                      ) : (
                        <div style={{ display: 'flex', gap: '4px' }}>
                          {getActionButtons(o)?.map((a) => (
                            <button key={a.action} type="button"
                              ...
                              style={{ padding: '3px 8px', fontSize: '11px' }}
                              onClick={() => { setActionId(o.id); setActionType(a.action); setActionReason(''); setReconnFee(''); }}>
                              {a.label}
                            </button>
                          ))}
                        </div>
              ...
              <tbody>
                {arrears.data.map((c) => (
                  <tr key={c.id}>
                    ...
                    <td>
                      {canManage && (
                        <button type="button" className="bill-btn bill-btn--danger"
                          style={{ padding: '3px 8px', fontSize: '11px' }}
