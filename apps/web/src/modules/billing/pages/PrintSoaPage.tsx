...
export default function PrintSoaPage() {
  ...
  useEffect(() => {
    if (!consumerId) { setError('No consumer specified.'); return; }
  ...
  const d = data as {
    consumer: { accountNumber: string; firstName: string; middleName?: string; lastName: string; address: string; barangay?: string; consumerType: string; status: string };
  ...
  const fullName = [d.consumer.lastName, d.consumer.firstName, d.consumer.middleName].filter(Boolean).join(', ');
          ...
          <tbody>
            {d.bills.map((b) => (
              <tr key={b.billNumber}>
                ...
                <td className="bp-center">{new Date(b.dueDate).toLocaleDateString('en-PH', { month: '2-digit', day: '2-digit', year: 'numeric' })}</td>
                <td className="bp-center" style={{ textTransform: 'capitalize' }}>{b.status}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
        ...
        {d.payments.length > 0 && (
          <>
            ...
            <table className="bill-print-table">
              ...
              <tbody>
                {d.payments.map((p) => (
                  <tr key={p.orNumber} style={p.status === 'voided' ? { textDecoration: 'line-through', color: '#888' } : undefined}>
                    <td className="bp-center">{p.orNumber}</td>
                    <td className="bp-center">{new Date(p.paymentDate).toLocaleDateString('en-PH', { month: '2-digit', day: '2-digit', year: 'numeric' })}</td>
                    <td className="bp-right">{formatPeso(p.totalAmount)}</td>
                    <td className="bp-center" style={{ textTransform: 'capitalize' }}>{p.paymentMethod.replace('_', ' ')}</td>
                    <td className="bp-center" style={{ textTransform: 'capitalize' }}>{p.status}</td>
                  </tr>
                ))}
              </tbody>
            </table>
