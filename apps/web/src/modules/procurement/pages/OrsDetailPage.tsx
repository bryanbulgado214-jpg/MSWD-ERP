...
export function OrsDetailPage() {
  ...
  if (state.status === 'loading')
    return (
      <div className="pr-page">
        <div className="pr-empty">Loading ORS...</div>
      </div>
    );
  ...
  return (
    <div className="pr-page">
      <Link to="/procurement/ors" className="pr-back">
        &larr; Back to ORS
      </Link>
      ...
      <div className="pr-detail-actions">
        <Link
          to={`/procurement/ors/${ors.id}/print`}
          className="pr-btn"
      ...
      <dl className="pr-detail-meta">
        ...
        <div>
          <dt>Fund Source</dt>
          <dd>
            {ors.fundSource.code} — {ors.fundSource.name}
          </dd>
        </div>
        <div>
          <dt>Responsibility Center</dt>
          <dd>
            {ors.responsibilityCenter.code} — {ors.responsibilityCenter.name}
          </dd>
        </div>
        {ors.accountCode && (
          <div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        >
          ...
          {canCreate &&
            isActive &&
            ['obligated', 'partially_payable', 'partially_paid'].includes(ors.status) && (
              <button
                className="pr-btn pr-btn--primary"
        ...
        {showChildForm && (
          <form
            ...
            style={{ background: '#f8f9fc', borderRadius: 8, padding: 16, marginBottom: 12 }}
          >
            <div
              style={{
                display: 'grid',
            ...
            <div className="pr-form-actions" style={{ marginTop: 12 }}>
              <button type="button" className="pr-btn" onClick={() => setShowChildForm(false)}>
                Cancel
              </button>
              <button type="submit" className="pr-btn pr-btn--primary" disabled={acting}>
                Add
              </button>
            </div>
          </form>
        ...
        {ors.children.length === 0 ? (
          <div className="pr-empty" style={{ padding: 24 }}>
            No disbursement or payment records yet.
          </div>
        ) : (
              ...
              <tbody>
                {ors.children.map((c) => (
                  <tr key={c.id}>
                    <td>
                      <span
                        className={`pr-badge pr-badge--${c.childType === 'payment' ? 'approved' : 'submitted'}`}
                      >
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        >
          ...
          {canAdjust &&
            isActive &&
            ['obligated', 'partially_payable', 'partially_paid', 'adjusted'].includes(
              ors.status,
            ) && (
              <button
                className="pr-btn"
        ...
        {showAdjForm && (
          ...
          >
            ...
            <div className="pr-form-actions" style={{ marginTop: 12 }}>
              <button type="button" className="pr-btn" onClick={() => setShowAdjForm(false)}>
                Cancel
              </button>
              <button type="submit" className="pr-btn pr-btn--primary" disabled={acting}>
                Add Adjustment
                  ...
                  <tr key={a.id}>
                    ...
                    <td
                      style={{
                        textAlign: 'right',
                        fontVariantNumeric: 'tabular-nums',
                        color: parseFloat(a.signedAmount) < 0 ? '#b42318' : '#067647',
                      }}
                    >
        ...
        <div className="pr-audit-trail__entries">
          ...
          {ors.requestingOfficeCertifier && ors.requestingOfficeCertifiedAt && (
            <div className="pr-audit-entry">
              ...
              <span className="pr-audit-entry__date">
                {new Date(ors.requestingOfficeCertifiedAt).toLocaleString()}
              </span>
            </div>
          )}
          {ors.budgetCertifier && ors.budgetCertifiedAt && (
            <div className="pr-audit-entry">
              ...
              <span className="pr-audit-entry__date">
                {new Date(ors.budgetCertifiedAt).toLocaleString()}
              </span>
            </div>
          )}
        </div>
