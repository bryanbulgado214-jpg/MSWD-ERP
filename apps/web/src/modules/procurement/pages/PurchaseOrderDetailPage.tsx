...
export function PurchaseOrderDetailPage() {
  ...
  function load() {
    ...
    getPurchaseOrder(id)
      ...
      .catch((e) =>
        setState({
          status: 'error',
          message: e instanceof ProcurementApiError ? e.message : 'Failed to load PO.',
        }),
      );
  }
  ...
  useEffect(() => {
    ...
    listCafs({ purchaseOrderId: state.data.id })
      .then(setLinkedCAFs)
      .catch(() => {});
  ...
  async function doCancel() {
    ...
    setError('');
    try {
      const updated = await cancelPurchaseOrder(
        state.data.id,
        state.data.version,
        remarks || undefined,
      );
  ...
  if (state.status === 'loading')
    return (
      <div className="pr-page">
        <div className="pr-empty">Loading PO...</div>
      </div>
    );
  ...
  return (
    <div className="pr-page">
      ...
      <div className="pr-detail-actions">
        <Link
          to={`/procurement/purchase-orders/${po.id}/print`}
          className="pr-btn"
