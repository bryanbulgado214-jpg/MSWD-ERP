...
type LoadState =
  { status: 'loading' } | { status: 'error'; message: string } | { status: 'loaded'; data: Caf[] };
    ...
    listCafs(statusFilter ? { status: statusFilter } : undefined)
      ...
      .catch((e) => {
        if (!cancelled)
          setState({
            status: 'error',
            message: e instanceof ProcurementApiError ? e.message : 'Failed to load CAFs.',
          });
      });
    return () => {
      cancelled = true;
            ...
            <tbody>
              {state.data.map((caf) => (
                <tr key={caf.id}>
                  ...
                  <td>
                    <Link
                      to={`/procurement/purchase-requests/${caf.purchaseRequest.id}`}
