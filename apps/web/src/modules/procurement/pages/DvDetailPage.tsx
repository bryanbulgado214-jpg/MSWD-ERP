...
export function DvDetailPage() {
  ...
  function load() {
    ...
    getDv(id)
      ...
      .catch((e) =>
        setState({ status: 'error', message: e instanceof ProcurementApiError ? e.message : 'Failed to load DV.' }),
      );
  }
  ...
  async function doRelease() {
    ...
    try {
      ...
      const checkDate = checkNumber ? window.prompt('Check date (YYYY-MM-DD):') ?? undefined : undefined;
  ...
  if (state.status === 'loading') return <div className="pr-page"><div className="pr-empty">Loading disbursement voucher...</div></div>;
  ...
  return (
    <div className="pr-page">
      <Link to="/procurement/dvs" className="pr-back">&larr; Back to DVs</Link>
      ...
      <dl className="pr-detail-meta">
        ...
        <div>
          ...
          <dd>
            <Link to={`/procurement/purchase-orders/${dv.purchaseOrder.id}`} className="pr-table__link">
        ...
        <div>
          <dt>Supplier</dt>
          <dd>{dv.supplier.name}{dv.supplier.tin ? ` (TIN: ${dv.supplier.tin})` : ''}</dd>
        </div>
        {dv.inspectionReport && (
          <div>
            <dt>Inspection Report</dt>
            <dd>
              <Link to={`/procurement/inspections/${dv.inspectionReport.id}`} className="pr-table__link">
        ...
        {dv.fundSource && (
          <div>
            <dt>Fund Source</dt>
            <dd>{dv.fundSource.code} — {dv.fundSource.name}</dd>
          </div>
        )}
        {dv.responsibilityCenter && (
          <div>
            <dt>Responsibility Center</dt>
            <dd>{dv.responsibilityCenter.code} — {dv.responsibilityCenter.name}</dd>
          </div>
        )}
        {dv.accountCode && (
          <div>
      ...
      <div style={{ background: '#f8f9fc', borderRadius: 8, padding: 16, marginTop: 16 }}>
        <h3 style={{ margin: '0 0 8px', fontSize: 13, fontWeight: 700, color: '#667085', textTransform: 'uppercase' }}>Particulars</h3>
        <p style={{ margin: 0, fontSize: 14, whiteSpace: 'pre-wrap' }}>{dv.particulars}</p>
      </div>

      {/* Financial Summary */}
      <div style={{ marginTop: 24 }}>
        <h3 style={{ fontSize: 15, fontWeight: 700, color: '#0f172a', margin: '0 0 12px' }}>Financial Summary</h3>
        <table className="pr-table" style={{ maxWidth: 400 }}>
          <tbody>
            <tr>
              <td style={{ fontWeight: 600 }}>Gross Amount</td>
              <td style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{formatPeso(dv.grossAmount)}</td>
            </tr>
            <tr>
              <td style={{ color: '#b91c1c' }}>Less: Tax</td>
              <td style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums', color: '#b91c1c' }}>({formatPeso(dv.taxAmount)})</td>
            </tr>
            <tr>
              <td style={{ color: '#b91c1c' }}>Less: Other Deductions</td>
              <td style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums', color: '#b91c1c' }}>({formatPeso(dv.otherDeductions)})</td>
            </tr>
            <tr style={{ borderTop: '2px solid #0f172a' }}>
              <td style={{ fontWeight: 700, fontSize: 15 }}>Net Amount</td>
              <td style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontWeight: 700, fontSize: 15, color: '#067647' }}>
      ...
      {(dv.checkNumber || dv.bankName) && (
        <div style={{ marginTop: 24 }}>
          <h3 style={{ fontSize: 15, fontWeight: 700, color: '#0f172a', margin: '0 0 12px' }}>Payment Details</h3>
          <dl className="pr-detail-meta">
            {dv.checkNumber && (
              <div>
                <dt>Check Number</dt>
        ...
        <div className="pr-audit-trail__entries">
          {dv.creator && (
            <div className="pr-audit-entry">
              ...
              <span className="pr-audit-entry__date">{new Date(dv.createdAt).toLocaleString()}</span>
            </div>
          )}
          {dv.certifier && dv.certifiedAt && (
            <div className="pr-audit-entry">
              ...
              <span className="pr-audit-entry__date">{new Date(dv.certifiedAt).toLocaleString()}</span>
            </div>
          )}
          {dv.approver && dv.approvedAt && (
            <div className="pr-audit-entry">
              ...
              <span className="pr-audit-entry__date">{new Date(dv.approvedAt).toLocaleString()}</span>
            </div>
          )}
          {dv.releaser && dv.releasedAt && (
            <div className="pr-audit-entry">
              ...
              <span className="pr-audit-entry__date">{new Date(dv.releasedAt).toLocaleString()}</span>
            </div>
          )}
        </div>
      </div>
