...
async function authFetchMutate(path: string, method: 'POST' | 'PUT' | 'PATCH' | 'DELETE', body?: unknown): Promise<Response> {
  const token = getAccessToken();
  const response = await fetch(`${API_BASE_URL}${path}`, {
    method,
    headers: {
...
export async function assignCategory(propertyRecordId: string, assetCategoryId: string): Promise<unknown> {
  const res = await authFetchMutate(`/asset/property-records/${propertyRecordId}/assign-category`, 'POST', { assetCategoryId });
  return res.json();
}
...
export async function createDepreciationRun(data: { periodMonth: number; periodYear: number }): Promise<DepreciationRun> {
...
export async function postDepreciationRun(id: string, version: number): Promise<DepreciationRun> {
  const res = await authFetchMutate(`/asset/depreciation-runs/${id}/post`, 'POST', { expectedVersion: version });
  return res.json();
}

export async function voidDepreciationRun(id: string, version: number): Promise<DepreciationRun> {
  const res = await authFetchMutate(`/asset/depreciation-runs/${id}/void`, 'POST', { expectedVersion: version });
  return res.json();
}
...
export async function approveTransfer(id: string, version: number): Promise<AssetTransfer> {
  const res = await authFetchMutate(`/asset/transfers/${id}/approve`, 'POST', { expectedVersion: version });
  return res.json();
}

export async function rejectTransfer(id: string, version: number, reason?: string): Promise<AssetTransfer> {
...
export async function completeTransfer(id: string, version: number): Promise<AssetTransfer> {
  const res = await authFetchMutate(`/asset/transfers/${id}/complete`, 'POST', { expectedVersion: version });
  return res.json();
}
...
export async function getDepreciationSchedule(categoryId?: string): Promise<DepreciationScheduleItem[]> {
...
export async function getPropertyRecords(): Promise<Array<{
  id: string;
