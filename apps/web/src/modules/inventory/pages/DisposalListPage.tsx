  ...
  return (
    <div className="inv-page">
      ...
      {state.status === 'loaded' && state.data.length === 0 && (
        <div className="inv-empty">No disposal requests found.</div>
      )}
      {state.status === 'loaded' && state.data.length > 0 && (
          ...
          <table className="inv-table">
            ...
            <tbody>
              {state.data.map((d) => (
                <tr key={d.id}>
                  <td>
                    <Link to={`/inventory/disposal/${d.id}`} className="inv-table__link">
                      {d.requestNumber}
                    </Link>
                  </td>
                  ...
                  <td>
                    <span className={`inv-badge inv-badge--${d.status}`}>
                      {d.status.replace(/_/g, ' ')}
                    </span>
                  </td>
                  <td className="inv-text-right inv-text-mono">
                    {d.totalAppraised ? formatPeso(d.totalAppraised) : '—'}
                  </td>
