  ...
  return (
    <div className="inv-page">
      ...
      {state.status === 'loaded' && state.data.length === 0 && (
        <div className="inv-empty">No property records found.</div>
      )}
      {state.status === 'loaded' && state.data.length > 0 && (
          ...
          <table className="inv-table">
            ...
            <tbody>
              {state.data.map((p) => (
                <tr key={p.id}>
                  <td>
                    <Link to={`/inventory/property-records/${p.id}`} className="inv-table__link">
                      {p.propertyNumber}
                    </Link>
                  </td>
                  <td>{p.description}</td>
                  <td>
                    <span className={`inv-badge inv-badge--${p.inventoryItem.classification}`}>
                      {p.inventoryItem.classification.replace('_', ' ')}
                    </span>
                  </td>
                  <td>
                    <span className={`inv-badge inv-badge--${p.condition}`}>
