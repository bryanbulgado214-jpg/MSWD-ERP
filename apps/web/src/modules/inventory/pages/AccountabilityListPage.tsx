...
export default function AccountabilityListPage() {
  ...
  return (
    <div className="inv-page">
      ...
      {state.status === 'loaded' && state.data.length === 0 && <div className="inv-empty">No records found.</div>}
      {state.status === 'loaded' && state.data.length > 0 && (
        <table className="inv-table">
          ...
          <tbody>
            {state.data.map((r) => (
              <tr key={r.id}>
                <td><Link to={`/inventory/accountability/${r.id}`} className="inv-table__link">{r.accountabilityNumber}</Link></td>
                <td><span className={`inv-badge inv-badge--${r.accountabilityType === 'par' ? 'ppe' : 'semi_expendable'}`}>{r.accountabilityType.toUpperCase()}</span></td>
                <td>{r.issuedTo.username}</td>
                <td>{new Date(r.issuedDate).toLocaleDateString()}</td>
                <td><span className={`inv-badge inv-badge--${r.status}`}>{r.status}</span></td>
