...
export default function StockReceiptListPage() {
  ...
  return (
    <div className="inv-page">
      ...
      {state.status === 'loaded' && state.data.length === 0 && <div className="inv-empty">No stock receipts found.</div>}
      {state.status === 'loaded' && state.data.length > 0 && (
        <table className="inv-table">
          ...
          <tbody>
            {state.data.map((r) => (
              <tr key={r.id}>
                <td><Link to={`/inventory/stock-receipts/${r.id}`} className="inv-table__link">{r.receiptNumber}</Link></td>
