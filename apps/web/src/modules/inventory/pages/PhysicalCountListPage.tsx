  ...
  return (
    <div className="inv-page">
      ...
      {state.status === 'loaded' && state.data.length === 0 && (
        <div className="inv-empty">No physical counts found.</div>
      )}
      {state.status === 'loaded' && state.data.length > 0 && (
          ...
          <table className="inv-table">
            ...
            <tbody>
              {state.data.map((c) => (
                <tr key={c.id}>
                  <td>
                    <Link to={`/inventory/physical-counts/${c.id}`} className="inv-table__link">
                      {c.countNumber}
                    </Link>
                  </td>
                  ...
                  <td>
                    <span className={`inv-badge inv-badge--${c.status}`}>
                      {c.status.replace('_', ' ')}
                    </span>
                  </td>
