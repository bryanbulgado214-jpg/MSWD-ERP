      ...
      {agencyDetail && (
        <>
          <div className="hr-toolbar" style={{ marginTop: 0 }}>
            <button type="button" className="hr-btn" onClick={() => setAgencyDetail(null)}>
              ...
              <table className="hr-table">
                  ...
                  {agencyDetail.records.map((r, i) => (
                    <tr key={i}>
                      ...
                      <td className="hr-text-mono" style={{ textAlign: 'right' }}>
                        {formatPeso(r.employeeShare)}
                      </td>
                      <td className="hr-text-mono" style={{ textAlign: 'right' }}>
                        {formatPeso(r.employerShare)}
                      </td>
                ...
                <tfoot>
                  <tr style={{ fontWeight: 700 }}>
                    ...
                    <td className="hr-text-mono" style={{ textAlign: 'right' }}>
                      {formatPeso(agencyDetail.records.reduce((s, r) => s + r.employeeShare, 0))}
                    </td>
                    <td className="hr-text-mono" style={{ textAlign: 'right' }}>
                      {formatPeso(agencyDetail.records.reduce((s, r) => s + r.employerShare, 0))}
                    </td>
                    <td className="hr-text-mono" style={{ textAlign: 'right' }}>
                      {formatPeso(agencyDetail.records.reduce((s, r) => s + r.total, 0))}
                    </td>
                  </tr>
                </tfoot>
              </table>
        ...
        <>
          {summary.runs.length > 0 && (
            <div style={{ marginBottom: 16, fontSize: 13, color: '#475467' }}>
              Based on {summary.runs.length} payroll run(s):{' '}
              {summary.runs.map((r) => r.runNumber).join(', ')}
            </div>
          )}

          {summary.agencies.length === 0 && (
            <div className="hr-empty">
              No remittance data found. Compute a payroll run first to generate deduction data.
            </div>
          )}
              ...
              {summary.agencies.map((a) => (
                ...
                >
                  ...
                  >
                    ...
                    <span className="hr-badge hr-badge--permanent">
                  ...
                  >
                    <div>
                      ...
                      <div className="hr-text-mono" style={{ fontSize: 15, fontWeight: 600 }}>
                        {formatPeso(a.totalEmployee)}
                      </div>
                    </div>
                    <div>
                      <div
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{ fontSize: 16, fontWeight: 700, color: '#344054' }}
                    >
                      {formatPeso(a.totalEmployee + a.totalEmployer)}
                    </span>
                  </div>
                  <div style={{ marginTop: 8, fontSize: 11, color: '#6941c6', textAlign: 'right' }}>
