...
export default function EmployeeDetailPage() {
  ...
  async function handleUpdate(ev: React.FormEvent) {
    ...
    try {
      await updateEmployee(state.data.id, {
        ...
        tin, sssGsisNumber, philhealthNumber, pagibigNumber,
  ...
  if (state.status === 'loading') return <div className="hr-page"><HrSubNav /><p>Loading...</p></div>;
  if (state.status === 'error') return <div className="hr-page"><HrSubNav /><div className="hr-error">{state.message}</div></div>;
  ...
  return (
    <div className="hr-page">
      <HrSubNav />
      <Link to="/hr" className="hr-back-link">&larr; Back to Employees</Link>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
        <h1 style={{ margin: '8px 0' }}>{e.lastName}, {e.firstName}{e.middleName ? ` ${e.middleName}` : ''}{e.suffix ? ` ${e.suffix}` : ''}</h1>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <span className={`hr-badge hr-badge--${e.employmentStatus}`}>{e.employmentStatus.replace('_', ' ')}</span>
          <span className={`hr-badge hr-badge--${e.employmentType}`}>{e.employmentType.replace('_', ' ')}</span>
        </div>
      </div>
      ...
      {!editing && (
        <>
          <div className="hr-detail-grid">
            <div className="hr-detail-item"><span className="hr-detail-label">Employee #</span><span className="hr-detail-value hr-text-mono">{e.employeeNumber}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">Position</span><span className="hr-detail-value">{e.position?.title ?? '--'}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">Department</span><span className="hr-detail-value">{e.department?.name ?? '--'}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">Basic Salary</span><span className="hr-detail-value hr-text-mono">{formatPeso(e.basicSalary)}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">Salary Grade / Step</span><span className="hr-detail-value">{e.salaryGrade !== null ? `SG ${e.salaryGrade} Step ${e.salaryStep ?? 1}` : '--'}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">Linked User</span><span className="hr-detail-value">{e.user?.username ?? '--'}</span></div>
          </div>

          <h3 className="hr-section-title">Personal Information</h3>
          <div className="hr-detail-grid">
            <div className="hr-detail-item"><span className="hr-detail-label">Date of Birth</span><span className="hr-detail-value">{e.dateOfBirth ? new Date(e.dateOfBirth).toLocaleDateString() : '--'}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">Gender</span><span className="hr-detail-value" style={{ textTransform: 'capitalize' }}>{e.gender || '--'}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">Civil Status</span><span className="hr-detail-value" style={{ textTransform: 'capitalize' }}>{e.civilStatus || '--'}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">Address</span><span className="hr-detail-value">{e.address || '--'}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">Contact</span><span className="hr-detail-value">{e.contactNumber || '--'}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">Email</span><span className="hr-detail-value">{e.email || '--'}</span></div>
          </div>

          <h3 className="hr-section-title">Government IDs</h3>
          <div className="hr-detail-grid">
            <div className="hr-detail-item"><span className="hr-detail-label">TIN</span><span className="hr-detail-value hr-text-mono">{e.tin || '--'}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">GSIS #</span><span className="hr-detail-value hr-text-mono">{e.sssGsisNumber || '--'}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">PhilHealth #</span><span className="hr-detail-value hr-text-mono">{e.philhealthNumber || '--'}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">Pag-IBIG #</span><span className="hr-detail-value hr-text-mono">{e.pagibigNumber || '--'}</span></div>
          </div>

          <h3 className="hr-section-title">Employment History</h3>
          <div className="hr-detail-grid">
            <div className="hr-detail-item"><span className="hr-detail-label">Date Hired</span><span className="hr-detail-value">{e.dateHired ? new Date(e.dateHired).toLocaleDateString() : '--'}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">Date Regularized</span><span className="hr-detail-value">{e.dateRegularized ? new Date(e.dateRegularized).toLocaleDateString() : '--'}</span></div>
            <div className="hr-detail-item"><span className="hr-detail-label">Date Separated</span><span className="hr-detail-value">{e.dateSeparated ? new Date(e.dateSeparated).toLocaleDateString() : '--'}</span></div>
            {e.separationReason && <div className="hr-detail-item"><span className="hr-detail-label">Separation Reason</span><span className="hr-detail-value">{e.separationReason}</span></div>}
          </div>

          {hasPermission('hr.employee.manage') && (
            <button type="button" className="hr-btn hr-btn--primary" onClick={() => startEdit(e)}>Edit Employee</button>
          )}
        ...
        <form className="hr-form" onSubmit={handleUpdate}>
          ...
          <fieldset className="hr-fieldset">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
              <div className="hr-field">
                <label>Status</label>
                <select value={employmentStatus} onChange={(ev) => setEmploymentStatus(ev.target.value)}>
                  <option value="active">Active</option><option value="resigned">Resigned</option><option value="retired">Retired</option>
                  <option value="terminated">Terminated</option><option value="on_leave">On Leave</option><option value="suspended">Suspended</option>
                </select>
              </div>
              ...
              <div className="hr-field"><label>Date Regularized</label><input type="date" value={dateRegularized} onChange={(ev) => setDateRegularized(ev.target.value)} /></div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
              ...
              <div className="hr-field"><label>Salary Step</label><input type="number" value={salaryStep} onChange={(ev) => setSalaryStep(ev.target.value)} /></div>
            </div>
            {(employmentStatus === 'resigned' || employmentStatus === 'retired' || employmentStatus === 'terminated') && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px' }}>
                <div className="hr-field"><label>Date Separated</label><input type="date" value={dateSeparated} onChange={(ev) => setDateSeparated(ev.target.value)} /></div>
                <div className="hr-field"><label>Separation Reason</label><input value={separationReason} onChange={(ev) => setSeparationReason(ev.target.value)} /></div>
              </div>
            )}
          </fieldset>

          <div className="hr-form-actions">
            <button type="button" className="hr-btn" onClick={() => setEditing(false)}>Cancel</button>
            <button type="submit" className="hr-btn hr-btn--primary" disabled={saving}>{saving ? 'Saving...' : 'Update Employee'}</button>
          </div>
        </form>
      )}
    </div>
