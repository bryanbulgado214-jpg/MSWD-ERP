    ...
    <div className="hr-page">
      ...
      <div className="hr-toolbar">
        <div className="hr-toolbar__filters" style={{ flexWrap: 'wrap', gap: 8 }}>
          ...
          {reportType === 'payroll-register' && (
            <select
              ...
              style={{ padding: '6px 10px', width: '100%', maxWidth: 240, boxSizing: 'border-box' }}
            >
              ...
              {runs
                ...
                .map((r) => (
                  <option key={r.id} value={r.id}>
                    {r.runNumber} — {r.payrollPeriod.name} ({r.status})
                  </option>
                ))}
            </select>
          )}
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          {data && (
            <button type="button" className="hr-btn" onClick={handlePrint}>
              Print
            </button>
          )}
        </div>
                ...
                <tbody>
                  {data.map((emp: any) => (
                    <tr key={emp.id}>
                      ...
                      <td>
                        <span
                          className={`hr-badge hr-badge--${emp.employmentStatus === 'active' ? 'active' : 'resigned'}`}
                        >
                      ...
                      <td className="hr-text-mono" style={{ textAlign: 'right' }}>
                        {formatPeso(emp.basicSalary)}
                      </td>
                      <td>
                        {emp.salaryGrade ?? '--'}/{emp.salaryStep ?? '--'}
                      </td>
      ...
      {reportType === 'payroll-register' && data && !Array.isArray(data) && (
        <div className="hr-report-print">
          ...
          <p className="hr-report-subtitle">
            {data.payrollPeriod?.name} | {fmtDate(data.payrollPeriod?.startDate)} –{' '}
            {fmtDate(data.payrollPeriod?.endDate)} | Pay Date:{' '}
            {fmtDate(data.payrollPeriod?.payDate)}
          </p>
          ...
          >
            <div className="hr-form" style={{ padding: 10, textAlign: 'center' }}>
              <div style={{ fontSize: 11, color: '#98a2b3', textTransform: 'uppercase' }}>
                Employees
              </div>
            ...
            <div className="hr-form" style={{ padding: 10, textAlign: 'center' }}>
              ...
              <div className="hr-text-mono" style={{ fontSize: 16, fontWeight: 700 }}>
                {formatPeso(data.totalGross)}
              </div>
            </div>
            <div className="hr-form" style={{ padding: 10, textAlign: 'center' }}>
              <div style={{ fontSize: 11, color: '#98a2b3', textTransform: 'uppercase' }}>
              ...
              >
                {formatPeso(data.totalDeductions)}
              </div>
            </div>
            <div className="hr-form" style={{ padding: 10, textAlign: 'center' }}>
              <div style={{ fontSize: 11, color: '#98a2b3', textTransform: 'uppercase' }}>
          ...
          {data.approver && (
            <p style={{ fontSize: 12, marginTop: 12, color: '#475467' }}>
              Approved by: {data.approver.username}
            </p>
          )}
          {data.creator && (
            <p style={{ fontSize: 12, color: '#475467' }}>Prepared by: {data.creator.username}</p>
          )}
        </div>
      )}
      ...
      {reportType === 'leave-summary' && data && data.balances && (
        <div className="hr-report-print">
          ...
          {data.balances.length === 0 && (
            <div className="hr-empty">No leave balances found for {data.year}.</div>
          )}
          {data.balances.length > 0 && (
            <div style={{ overflowX: 'auto' }}>
              <table className="hr-table">
      ...
      {reportType === 'attendance' && data && data.employees && (
        <div className="hr-report-print">
          <h3 className="hr-report-title">
            Attendance Summary — {MONTHS[data.month - 1]} {data.year}
          </h3>
                ...
                <tfoot>
                  <tr style={{ fontWeight: 700 }}>
                    ...
                    <td style={{ textAlign: 'right' }}>
                      {data.employees.reduce((s: number, e: any) => s + e.dtrCount, 0)}
                    </td>
                    <td style={{ textAlign: 'right' }}>
                      {data.employees.reduce((s: number, e: any) => s + e.daysPresent, 0)}
                    </td>
                    <td style={{ textAlign: 'right' }}>
                      {data.employees.reduce((s: number, e: any) => s + e.daysAbsent, 0)}
                    </td>
                    <td className="hr-text-mono" style={{ textAlign: 'right' }}>
                      {data.employees.reduce((s: number, e: any) => s + e.totalLate, 0)}
                    </td>
                    <td className="hr-text-mono" style={{ textAlign: 'right' }}>
                      {data.employees.reduce((s: number, e: any) => s + e.totalUndertime, 0)}
                    </td>
                    <td className="hr-text-mono" style={{ textAlign: 'right' }}>
                      {data.employees.reduce((s: number, e: any) => s + e.totalOvertime, 0)}
                    </td>
                    <td className="hr-text-mono" style={{ textAlign: 'right' }}>
                      {data.employees.reduce((s: number, e: any) => s + e.totalHoursWorked, 0)}
                    </td>
                  </tr>
                </tfoot>
              </table>
