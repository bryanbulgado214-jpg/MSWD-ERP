...
export default function CompensationPage() {
  ...
  function startNewAllow() {
    ...
    setAFixed(true);
    setFormError('');
    setShowAllowForm(true);
  }
  function startEditAllow(a: AllowanceType) {
    setEditingAllowId(a.id);
    setACode(a.code);
    setAName(a.name);
    setADefault(String(Number(a.defaultAmount)));
    setATaxable(a.isTaxable);
    setAFixed(a.isFixed);
    setFormError('');
    setShowAllowForm(true);
  }
  ...
  async function handleAllowSubmit(ev: React.FormEvent) {
    ...
    try {
      if (editingAllowId) {
        await updateAllowanceType(editingAllowId, {
          name: aName,
          isTaxable: aTaxable,
          isFixed: aFixed,
          ...(aDefault ? { defaultAmount: Number(aDefault) } : {}),
        });
      } else {
      ...
      setFormError(e instanceof HrApiError ? e.message : 'Failed.');
    } finally {
      setSaving(false);
    }
  }
  ...
  function startEditDed(d: DeductionType) {
    setEditingDedId(d.id);
    setDCode(d.code);
    setDName(d.name);
    setDCategory(d.category);
    setDIsPct(d.isPercentage);
    setDErShare(String(Number(d.employerShare)));
    setDEeShare(String(Number(d.employeeShare)));
    setFormError('');
    setShowDedForm(true);
  }
    ...
    <div className="hr-page">
      ...
      <div className="hr-toolbar">
        <div className="hr-toolbar__filters">
          <button
            type="button"
            className={`hr-btn${tab === 'allowances' ? ' hr-btn--primary' : ''}`}
          ...
          <button
            type="button"
            className={`hr-btn${tab === 'deductions' ? ' hr-btn--primary' : ''}`}
        ...
        {canManage && tab === 'allowances' && (
          <button type="button" className="hr-btn hr-btn--primary" onClick={startNewAllow}>
            + New Allowance Type
          </button>
        )}
        {canManage && tab === 'deductions' && (
          <button type="button" className="hr-btn hr-btn--primary" onClick={startNewDed}>
            + New Deduction Type
          </button>
        )}
      </div>
      ...
      {showAllowForm && (
        <form className="hr-form" onSubmit={handleAllowSubmit} style={{ marginBottom: 16 }}>
          ...
          >
            ...
            <div className="hr-field">
              ...
              <input
                ...
                step="0.01"
                value={aDefault}
                onChange={(e) => setADefault(e.target.value)}
              />
            </div>
            <div className="hr-field">
              <label>
                <input
                  type="checkbox"
                  checked={aTaxable}
                  onChange={(e) => setATaxable(e.target.checked)}
                />{' '}
                Taxable
              </label>
            </div>
            <div className="hr-field">
              <label>
                <input
                  type="checkbox"
                  checked={aFixed}
                  onChange={(e) => setAFixed(e.target.checked)}
                />{' '}
                Fixed
              </label>
            </div>
          ...
          <div className="hr-form-actions">
            <button type="button" className="hr-btn" onClick={() => setShowAllowForm(false)}>
              Cancel
            </button>
            <button type="submit" className="hr-btn hr-btn--primary" disabled={saving}>
              {saving ? 'Saving...' : editingAllowId ? 'Update' : 'Create'}
            </button>
          </div>
        </form>
      )}
      ...
      {showDedForm && (
        <form className="hr-form" onSubmit={handleDedSubmit} style={{ marginBottom: 16 }}>
          ...
          <div
            style={{
              display: 'grid',
              gridTemplateColumns: '120px 1fr 140px 120px 120px 100px',
              gap: 12,
              alignItems: 'end',
            }}
          >
            <div className="hr-field">
              ...
              <input
                ...
                disabled={!!editingDedId}
              />
            </div>
            <div className="hr-field">
              <label>Name *</label>
              <input required value={dName} onChange={(e) => setDName(e.target.value)} />
            </div>
            <div className="hr-field">
            ...
            <div className="hr-field">
              <label>ER Share {dIsPct ? '%' : '₱'}</label>
              <input
                type="number"
                step="0.0001"
                value={dErShare}
                onChange={(e) => setDErShare(e.target.value)}
              />
            </div>
            <div className="hr-field">
              <label>EE Share {dIsPct ? '%' : '₱'}</label>
              <input
                type="number"
                step="0.0001"
                value={dEeShare}
                onChange={(e) => setDEeShare(e.target.value)}
              />
            </div>
            <div className="hr-field">
              <label>
                <input
                  type="checkbox"
                  checked={dIsPct}
                  onChange={(e) => setDIsPct(e.target.checked)}
                />{' '}
                % Based
              </label>
            </div>
          ...
          <div className="hr-form-actions">
            <button type="button" className="hr-btn" onClick={() => setShowDedForm(false)}>
              Cancel
            </button>
            <button type="submit" className="hr-btn hr-btn--primary" disabled={saving}>
              {saving ? 'Saving...' : editingDedId ? 'Update' : 'Create'}
            </button>
          </div>
        </form>
      )}
      ...
      {tab === 'allowances' && !loading && (
        <>
          {allowTypes.length === 0 && (
            <div className="hr-empty">No allowance types configured.</div>
          )}
          {allowTypes.length > 0 && (
                ...
                <tbody>
                  {allowTypes.map((a) => (
                    <tr key={a.id}>
                      ...
                      <td>
                        <span
                          className={`hr-badge hr-badge--${a.isActive ? 'active' : 'resigned'}`}
                        >
                ...
                <tbody>
                  {dedTypes.map((d) => (
                    <tr key={d.id}>
                      ...
                      <td>
                        <span
                          className={`hr-badge hr-badge--${d.category === 'mandatory' ? 'permanent' : d.category === 'loan' ? 'contractual' : 'casual'}`}
                        >
                          {d.category}
                        </span>
                      </td>
                      ...
                      <td className="hr-text-mono" style={{ textAlign: 'right' }}>
                        {d.isPercentage
                          ? `${Number(d.employerShare)}%`
                          : formatPeso(d.employerShare)}
                      </td>
                      <td className="hr-text-mono" style={{ textAlign: 'right' }}>
                        {d.isPercentage
                          ? `${Number(d.employeeShare)}%`
                          : formatPeso(d.employeeShare)}
                      </td>
                      <td style={{ textAlign: 'right' }}>{d._count.employeeDeductions}</td>
                      <td>
                        <span
                          className={`hr-badge hr-badge--${d.isActive ? 'active' : 'resigned'}`}
                        >
